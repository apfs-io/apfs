...
> job-based workflows — see [WORKFLOW.md](WORKFLOW.md). The workflow `version`
> field is an object-freshness revision (compared to `Meta.ManifestVersion`),
> not a protocol identifier. Workflows can be applied automatically on service
> start via [INITIALIZATION.md](INITIALIZATION.md).
